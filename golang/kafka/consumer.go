...
func init() {
	...
	flag.StringVar(&version, "version", "0.10.2.0", "Kafka cluster version")
...
func main() {
	...
	// sarama.NewConsumer() 这个和 NewConsumerGroup 的区别是？
	...
	go func() {
		...
		for {
			...
			//}
			if pc, err := client.ConsumePartition(topic, 0, sarama.OffsetNewest); err != nil {
				log.Panicf("Error from consumer: %v", err)
			} else {
